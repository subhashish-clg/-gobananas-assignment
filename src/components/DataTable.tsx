...
const rows = [
  ...
  createData(13, "Oreo", 437, 18.0, 63, -10.0),
];
...
export default function DataTable() {
    ...
    setSearchMatches(
      rows.filter((row) => {
        ...
        if (searchedVal) {
          ...
          properties.forEach((property: keyof Data) => {
            if (
              row[property]
                .toString()
                .toLowerCase()
  ...
  const visibleRows = React.useMemo(() => {
    const rowsToBeDisplayed =
      searchedMatches.length > 0 ? searchedMatches : rows;

    return stableSort(rowsToBeDisplayed, getComparator(order, orderBy)).slice(
      page * rowsPerPage,
      page * rowsPerPage + rowsPerPage
    );
  }, [searchedMatches, order, orderBy, page, rowsPerPage]);
